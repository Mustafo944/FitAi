...
export default function SettingsPage() {
  ...
  const {
    ...
    resetOnboarding, setAnalysis, setDietPlan, setWorkoutPlan,
  } = useUserStore()
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-24">
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="bg-[#111] border border-white/8 rounded-2xl p-5 mb-4">
          <div className="text-xs text-[#c8f55a] font-semibold mb-4">👤 {t('settings_name')} / {t('settings_surname')}</div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">{t('settings_name')}</label>
              <input
                type="text"
                value={userName}
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1 block">{t('settings_surname')}</label>
              <input
                type="text"
                value={userSurname}
        ...
        {/* Language */}
        <div className="bg-[#111] border border-white/8 rounded-2xl p-5 mb-4">
          <div className="text-xs text-[#c8f55a] font-semibold mb-4">🌐 {t('settings_language')}</div>
        ...
        {/* Theme */}
        <div className="bg-[#111] border border-white/8 rounded-2xl p-5 mb-4">
          <div className="text-xs text-[#c8f55a] font-semibold mb-4">🎨 {t('settings_theme')}</div>
          <div className="flex gap-3">
            {(['dark', 'light'] as const).map((th) => (
              ...
              >
                {th === 'dark' ? `🌙 ${t('settings_theme_dark')}` : `☀️ ${t('settings_theme_light')}`}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#111] border border-white/8 rounded-2xl p-5 mb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="text-xs text-[#c8f55a] font-semibold">📋 {t('settings_profile')}</div>
            <button
          ...
          {!editingProfile ? (
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[#0a0a0a] rounded-xl p-3">
                <div className="text-[10px] text-gray-500 uppercase tracking-wider">{t('onboarding_height')}</div>
                <div className="text-white font-semibold">{onboardingData.height || '—'} sm</div>
              </div>
              <div className="bg-[#0a0a0a] rounded-xl p-3">
                <div className="text-[10px] text-gray-500 uppercase tracking-wider">{t('onboarding_weight')}</div>
                <div className="text-white font-semibold">{onboardingData.weight || '—'} kg</div>
              </div>
              <div className="bg-[#0a0a0a] rounded-xl p-3">
                <div className="text-[10px] text-gray-500 uppercase tracking-wider">{t('onboarding_age')}</div>
                <div className="text-white font-semibold">{onboardingData.age || '—'}</div>
              </div>
              <div className="bg-[#0a0a0a] rounded-xl p-3">
                <div className="text-[10px] text-gray-500 uppercase tracking-wider">{t('onboarding_gender')}</div>
                <div className="text-white font-semibold">
                  {onboardingData.gender === 'male' ? t('onboarding_gender_male') : onboardingData.gender === 'female' ? t('onboarding_gender_female') : '—'}
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">{t('onboarding_height')}</label>
                ...
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">{t('onboarding_weight')}</label>
                  <input type="number" value={localWeight} onChange={(e) => setLocalWeight(e.target.value)}
                    className="w-full bg-[#0a0a0a] border border-white/10 text-white px-4 py-3 rounded-xl text-sm focus:outline-none focus:border-[#c8f55a]" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">{t('onboarding_age')}</label>
                  <input type="number" value={localAge} onChange={(e) => setLocalAge(e.target.value)}
                    className="w-full bg-[#0a0a0a] border border-white/10 text-white px-4 py-3 rounded-xl text-sm focus:outline-none focus:border-[#c8f55a]" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">{t('onboarding_gender')}</label>
                  <div className="flex gap-2">
                    {(['male', 'female'] as Gender[]).map((g) => (
                      <button key={g} onClick={() => setLocalGender(g)}
                        ...
                        {g === 'male' ? `🚹 ${t('onboarding_gender_male')}` : `🚺 ${t('onboarding_gender_female')}`}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <label className="text-xs text-gray-500 mb-2 block">{t('onboarding_goal_title')}</label>
                <div className="grid grid-cols-2 gap-2">
                  {GOALS.map((g) => (
                    <button key={g.value} onClick={() => setLocalGoal(g.value as Goal)}
